...
export function useSettings() {
    ...
    const [freeGamesNotificationsEnabled, setFreeGamesNotificationsEnabledState] = useState(true);
    const [calendarNotificationsEnabled, setCalendarNotificationsEnabledState] = useState(true);
    const [pomoBlockEnabled, setPomoBlockEnabledState] = useState(true);
    const [universalInfoBoxEnabled, setUniversalInfoBoxEnabledState] = useState(true);
    const [universalInfoBoxHotkey, setUniversalInfoBoxHotkeyState] = useState("none");
    ...
    const loadSettings = useCallback(async () => {
        ...
        setAiShowThinking((ai.aiShowThinking as unknown as boolean | undefined) !== false);

        // KPSS settings
        const kpss = await syncGet<unknown>(["kpssGoalType", "kpssTargetNet", "kpssTargetScore"]);
    ...
    const handleUniversalInfoBoxHotkeyChange = useCallback(async (hotkey: string) => {
    ...
    const handleKpssGoalTypeChange = useCallback(async (type: "net" | "score") => {
    ...
    const handleDetoxLimitsChange = useCallback(async (limits: Record<string, number>) => {
