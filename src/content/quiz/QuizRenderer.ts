/**
 * QuizRenderer.ts
 * Shadow DOM DOM construction & overlay UI rendering for in-page AI quizzes.
 * Security: createElement + textContent only — NO innerHTML for user strings (AGENTS.md 4.4).
 */

import { contentLog } from "@/content/contentLogger.js";
import type { QuizQuestion } from "./QuizParser.js";
import { saveQuizStats } from "./QuizStorage.js";
...
export function setQuizState(questions: QuizQuestion[]): void {
...
export function isPanelOpen(): boolean {
  return !!document.getElementById("lifos-quiz-panel-host");
}

/** Shows the small floating trigger button (bottom-right). */
export function showTrigger(): void {
...
function renderPanel(shadow: ShadowRoot): void {
  shadow.innerHTML = ""; // Clear root (safe shadow root control)
...
function renderQuestionView(panel: HTMLElement): void {
  ...
  // Explanation box — shown AFTER options once answered
  if (showResult) {
...
function renderResultView(panel: HTMLElement): void {
  ...
  if (total > 0) {
    void saveQuizStats(correct, total);
  }
}

/** Renders text with LaTeX-ish $...$ math strings as inline math elements. */
