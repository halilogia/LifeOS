...
export class ChromeStoragePrayerCacheRepository implements IPrayerCacheRepository {
  async getMonthCalendar(storageKey: string): Promise<Record<string, DayPrayerData> | null> {
  ...
  async setMonthCalendar(storageKey: string, data: Record<string, DayPrayerData>): Promise<void> {
    return new Promise((resolve) => {
