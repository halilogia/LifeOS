...
| `src/components/citypulse/`                    | Şehir etkinlikleri UI parçaları    | EventHubsBar.tsx, CityPulseFilterBar.tsx, CityEventCard.tsx                                                                                                                                                                        | State                                 |
...
| `src/components/routines/`                     | Alışkanlık zinciri ve Duolingo alev UI parçaları | RoutineStreakCard.tsx, FlameIcon.tsx, RoutineHeatmap.tsx                                                                                                                                                                          | State                                 |
...
| `src/services/`                                | Dış dünya iletişimi                | Network fetch, chrome.storage, AI servisleri (incl. aichat/agentTools.ts), kpss/, stock/, errorReportService                                                                                                                     | JSX                                   |
...
| `src/components/aichat/`                       | Newtab AI Asistan alt bileşenleri  | AiChatHeaderBar, AiChatMessageItem, AiChatInputToolbar, AiChatHistoryDrawer, ClarificationCard, QueuedMessagesBar, AiActionBadge                                                                                                    | State                                 |
| `src/domain/repositories/`                     | Depo arayüzleri (contract)         | ITodoRepository, IKpssRepository, IUserSyncProfileRepository, IChatSessionRepository                                                                                                              | Implementasyon                        |
| `src/domain/services/`                         | Saf hesaplama servisleri           | KpssCalculatorService, SrsService, TaskService, routineStreakCalculator                                                                                                                            | chrome.*                              |
| `src/domain/constants/`                        | Sabit veriler                      | kpssCurriculum, kpssConstants, TurkeyProvincePaths, history/ (kurtulusSavasiUnit — 21 adım)                                                                                                       | —                                     |
| `src/infrastructure/persistence/repositories/` | chrome.storage implementasyonları  | ChromeStorage*Repository (incl. ChromeStorageUserSyncProfileRepository, ChromeStorageChatSessionRepository)                                                                                      | UI                                    |
| `src/infrastructure/api/`                      | Google API client'ları             | GoogleTasksApi, GoogleDriveApi, GoogleAuthApi                                                                                                                                                     | —                                     |
| `src/services/aichat/`                         | AI Chat Servisleri                 | chatSessionService, fileAttachmentService (unpdf evrensel PDF), providers, systemPrompt, actionExecutor, registry/ (Extensible AI Feature Registry + 10 modüler eklenti), memoryRepository | JSX                                   |
| `src/sidepanel/`                               | Side Panel Web Copilot             | SidePanelApp, SidePanelHeader, SidePanelHistoryDrawer, SidePanelTabBar, SidePanelChips, SidePanelMessages, SidePanelInputBar, CommandSuggestionMenu, QueuedMessagesBar, useSidePanelChat, useChatSession, useAgentBridge | —                                     |
...
| `src/content/`                                 | Content script'ler                 | contentMain (giriş), contentLogger, infobox/, detox/, agent/ (pageContextExtractor, elementScanner, actionExecutor, domAgentEngine barrel), volume/ (volumeBooster), whatsapp/, telegram/, quiz/, rssFeedDiscovery                                                                             | —                                     |
...
| `src/utils/logger.ts`                          | Merkezi loglama                    | Tekil `logger` singleton: debug → yalnızca console; log/warn/error/info → 300'lük ring buffer (`logger_entries`), yazmalar 200ms coalescing ile birleştirilir, `error` anında flush edilir                                          | —                                     |
| `src/types/`                                   | Tip tanımları                      | types.ts, kpss.ts, stock.ts, bist.ts...                                                                                                                                                                                           | —                                     |
| `src/services/*/prompts/`                      | AI prompt kaynakları (.md)         | `?raw` ile import edilen Markdown prompt şablonları (kpss: base-rules + 6 ders, stock: 3, aichat: system-prompt). Kod dosyası değil — `findDeadFiles` bunları "boş klasör" saymamalı                                                                  | —                                     |
...
| AIChatView            | aiChatService, aiFeatureRegistry                          | sync+local | aichat/ (11)                                                        |
...
| -------------------------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| `findDeadFiles.mjs`        | Ölü dosya / boş klasör denetimi               | Hiçbir yerden import edilmeyen `.ts/.tsx/.css` dosyalarını, boş klasörleri ve `public/` referanssız asset'leri raporlar. **Boş klasör tespiti her dosya türünü sayar** (`.md` dahil) — aksi halde `?raw` prompt klasörleri yanlışlıkla "boş" görünür | 0 = temiz (kural 6.4)           |
