...
export function HifizView({ lang }: HifizViewProps) {
  ...
  // Tab views
  const [subView, setSubView] = useState<'memorizations' | 'yeterlikler'>('memorizations');
  ...
  // Mushaf Viewer Modal
  const [activeMushafItem, setActiveMushafItem] = useState<HifizItem | null>(null);
  const [currentPageIndex, setCurrentPageIndex] = useState(0);

  // Yeterlik Info Modal
  const [activeYeterlik, setActiveYeterlik] = useState<{ title: string; description: string } | null>(null);
  ...
  const memorizedCount = hifizProgress.filter((p) => p.status === 'memorized').length;
  const inProgressCount = hifizProgress.filter((p) => p.status === 'in_progress').length;
  ...
  INITIAL_HIFIZ_ITEMS.forEach((item) => {
    ...
    if (itemProgress) {
      if (itemProgress.status === 'memorized') {
        ...
        memorizedPages += itemProgress.pageStatuses.filter((s) => s === 'memorized').length;
      }
    }
  });

  const overallPercent = totalPages > 0 ? Math.round((memorizedPages / totalPages) * 100) : 0;
  ...
  const yeterliklerPercent = totalYeterliklerCount > 0 ? Math.round((completedYeterliklerCount / totalYeterliklerCount) * 100) : 0;
  ...
  const handleCycleStatus = async (itemId: string) => {
    ...
    if (idx === -1) {
      ...
      if (progress[idx].pageStatuses) {
        progress[idx].pageStatuses = progress[idx].pageStatuses?.map(() => nextStatus);
      }
    }
  ...
  const handleToggleYeterlik = async (index: number) => {
    ...
    if (currentCompleted.includes(index)) {
      next = currentCompleted.filter((i) => i !== index);
    } else {
      next = [...currentCompleted, index];
    }
  ...
  return (
    <div id="hifiz-view" className="view-content active">
      <div className="hifiz-container">
          ...
          <div id="hifiz-main-content" className="hifiz-sub-view active">
            ...
            <div className="hifiz-overall-progress-container">
              ...
              <div className="hifiz-overall-bar">
                <div id="hifiz-overall-fill" className="hifiz-overall-fill" style={{ width: `${overallPercent}%` }}></div>
              </div>
            </div>
            ...
            <div className="hifiz-stats">
              <div className="hifiz-stat-card">
                ...
                <div className="stat-info">
                  ...
                  <span id="hifiz-stat-memorized-count" className="stat-value">{memorizedCount}</span>
                </div>
              </div>
              ...
              <div className="hifiz-stat-card">
                ...
                <div className="stat-info">
                  ...
                  <span id="hifiz-stat-progress-count" className="stat-value">{inProgressCount}</span>
                </div>
              </div>
              ...
              <div className="hifiz-stat-card">
                ...
                <div className="stat-info">
                  ...
                  <span id="hifiz-stat-total-count" className="stat-value">{totalCount}</span>
                </div>
              </div>
            </div>
            ...
            <div id="hifiz-grid" className="hifiz-grid">
              {filteredItems.map((item) => {
                ...
                const pageStatuses = itemProgress.pageStatuses || new Array(totalPagesCount).fill('not_started');
                const memorizedPagesCount = pageStatuses.filter((s) => s === 'memorized').length;
                const pagePercent = Math.round((memorizedPagesCount / totalPagesCount) * 100);

                const statusText = t[`hifiz_status_${itemProgress.status}` as keyof typeof t] || itemProgress.status;
                const catLabel = t[`hifiz_cat_${item.category}` as keyof typeof t] || item.category;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="hifiz-card-body">
                      <h3>{item.title}</h3>
                      {item.description && <p className="hifiz-desc">{item.description}</p>}

                      {totalPagesCount > 1 && (
                        <div className="hifiz-pages-container">
                          <div className="hifiz-progress-track">
                            <div className="hifiz-progress-fill" style={{ width: `${pagePercent}%` }}></div>
                          </div>
                          ...
                          <div className="hifiz-progress-text">
                            {memorizedPagesCount}/{totalPagesCount} {t.hifiz_progress_pages}
                          </div>
                        </div>
                      )}
                    </div>
        ...
        {subView === 'yeterlikler' && (
          <div id="yeterlikler-content" className="hifiz-sub-view active">
            ...
            <div className="hifiz-overall-progress-container">
              <div className="hifiz-overall-info">
                <span>{lang === 'tr' ? 'Müfredat İlerlemesi' : 'Checklist Progress'}</span>
                <span id="yeterlikler-overall-percent">{yeterliklerPercent}%</span>
              </div>
              <div className="hifiz-overall-bar">
                <div id="yeterlikler-overall-fill" className="hifiz-overall-fill" style={{ width: `${yeterliklerPercent}%` }}></div>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        setActiveYeterlik({ title: item.title, description: item.description });
                      }}
                    >
      ...
      {/* Mushaf Viewer Modal */}
      {activeMushafItem && activeMushafItem.pages && activeMushafItem.pages.length > 0 && (
          ...
          <div className="hifiz-modal-content" onClick={(e) => e.stopPropagation()}>
            <header className="settings-header">
              ...
              <div style={{ display: 'flex', gap: '15px', alignItems: 'center', marginRight: '10px' }}>
                <span id="hifiz-page-info" style={{ fontSize: '0.95rem', color: 'var(--text-secondary)', fontWeight: 600, background: 'rgba(255,255,255,0.05)', padding: '4px 10px', borderRadius: '8px' }}>
                  {currentPageIndex + 1} / {activeMushafItem.pages.length}
                </span>
                <button className="close-btn" onClick={closeMushaf} style={{ margin: 0, fontSize: '1.8rem' }}>&times;</button>
              </div>
            </header>
            <div className="hifiz-image-body">
              ...
              <button
                id="hifiz-next-page"
                className="hifiz-nav-btn"
                disabled={currentPageIndex === activeMushafItem.pages.length - 1}
                onClick={() => setCurrentPageIndex((prev) => Math.min(activeMushafItem.pages!.length - 1, prev + 1))}
              >
                &gt;
              </button>
            </div>
      ...
      {activeYeterlik && (
        <div className="settings-panel active" onClick={() => setActiveYeterlik(null)}>
          <div className="settings-content" style={{ maxWidth: '500px' }} onClick={(e) => e.stopPropagation()}>
            <header className="settings-header">
              ...
              <button className="close-btn" onClick={() => setActiveYeterlik(null)}>&times;</button>
            </header>
            <div className="note-editor-body" style={{ padding: '24px' }}>
              <p id="yeterlik-modal-description" style={{ fontSize: '1.1rem', lineHeight: 1.6, color: 'var(--text-primary)' }}>
                {activeYeterlik.description}
              </p>
            </div>
            <div className="settings-footer">
              <button className="settings-add-btn" style={{ width: 'auto', padding: '0 30px' }} onClick={() => setActiveYeterlik(null)}>
