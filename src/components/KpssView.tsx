...
export function KpssView({ lang }: KpssViewProps) {
  ...
  // Detail Modal
  const [activeTopic, setActiveTopic] = useState<{ title: string; description: string } | null>(null);
  ...
  const handleToggleTopic = async (topic: string) => {
    const progressItem = kpssProgress.find((p) => p.subject === currentSubject && p.topic === topic);
      ...
      <div className="kpss-container">
        ...
        <div className="kpss-daily-stats-section">
          ...
          <div className="kpss-chart-container">
            <canvas ref={canvasRef} id="kpss-history-chart" style={{ display: showChartPlaceholder ? 'none' : 'block' }}></canvas>
        ...
        <div className="kpss-content">
          <div id="kpss-topic-list" className="kpss-topic-list">
            {topics.map((t) => {
              const progress = kpssProgress.find((p) => p.subject === currentSubject && p.topic === t.title);
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setActiveTopic({ title: t.title, description: t.description });
                    }}
                  >
        ...
        <div className="kpss-progress-bar-container">
          ...
          <div className="kpss-progress-track">
            <div id="kpss-progress-fill" className="kpss-progress-fill" style={{ width: `${progressPercentage}%` }}></div>
          </div>
        </div>
      </div>
        ...
        <div className="settings-panel active" onClick={() => setActiveTopic(null)}>
          <div className="settings-content" style={{ maxWidth: '500px' }} onClick={(e) => e.stopPropagation()}>
            <header className="settings-header">
              ...
              <button className="close-btn" onClick={() => setActiveTopic(null)}>&times;</button>
            </header>
            <div className="note-editor-body" style={{ padding: '24px' }}>
              <h4 style={{ fontSize: '1.2rem', color: 'var(--accent-color)', marginBottom: '10px' }}>{activeTopic.title}</h4>
              <p style={{ fontSize: '1.1rem', lineHeight: 1.6, color: 'var(--text-primary)' }}>{activeTopic.description}</p>
            </div>
            <div className="settings-footer">
              <button className="settings-add-btn" style={{ width: 'auto', padding: '0 30px' }} onClick={() => setActiveTopic(null)}>
