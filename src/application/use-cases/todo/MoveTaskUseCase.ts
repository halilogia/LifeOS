...
import { moveTaskWithStatus, getUpdatedStatuses } from "../../../domain/services/TaskService.js";
...
export class MoveTaskUseCase {
    ...
    async moveToStatus(request: MoveTaskStatusRequest): Promise<MoveTaskResponse> {
        const allTodos = await this.todoRepo.getAll();
        const updated = moveTaskWithStatus(allTodos, request.index, request.newStatus);

        // Sync to Google Tasks if changed and synced
        if (updated !== allTodos && request.index >= 0 && request.index < allTodos.length) {
    ...
    async moveByDirection(request: MoveTaskDirectionRequest): Promise<MoveTaskResponse> {
        const allTodos = await this.todoRepo.getAll();
        const newStatus = getUpdatedStatuses(allTodos, request.index, request.direction);
