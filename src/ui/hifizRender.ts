...
export function createCardHTML(
  ...
  const pageStatuses =
    itemProgress.pageStatuses || new Array(totalPages).fill("not_started");
...
export function updateHifizStats(progress: HifizProgress[]) {
  ...
  if (oPercent) {
    oPercent.textContent = `${overallPercent}%`;
  }
  if (oFill) {
    oFill.style.width = `${overallPercent}%`;
  }
  ...
  if (yPercent) {
    yPercent.textContent = `${overallPercent}%`;
  }
  if (yFill) {
    yFill.style.width = `${overallPercent}%`;
  }
}
