      ...
      generateBundle(_, bundle) {
        for (const entryName of ['content.js', 'background.js']) {
          ...
          for (const name of queue) {
            ...
            if (chunk && chunk.type === 'chunk') {
              for (const dep of chunk.imports) {
                if (!visited.has(dep) && dep.startsWith('assets/') && !dep.includes('.css')) {
                  queue.push(dep);
                }
              }
            }
          ...
          for (const impName of toInline.reverse()) {
            ...
            // Wrap in IIFE
            entryChunk.code = `(function(){${inlineCode}})();\n` + entryChunk.code;
            ...
            entryChunk.code = entryChunk.code.replace(
              new RegExp(`import\\s*(?:\\{[^}]*\\}|[^;]+?)\\s*from\\s*["'][^"']*${escapedName}["'];?\\s*`, 'g'),
              ''
            );

            // Remove from imports list but KEEP the chunk in bundle
            entryChunk.imports = entryChunk.imports.filter(i => !visited.has(i));
          }
